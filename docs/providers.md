...
2. **Custom-command presentation** — OpenCode sends custom commands through a
   ...
   provider/backend. See [Compatibility](reference.md#compatibility) in the
   reference, and the changelog entry for v0.6.6, for the historical findings.
...
1. Point an OpenCode config at the plugin (see
   [Development](reference.md#development) in the reference) and
