...
- **New defaults for unattended work: unlimited auto-continue turns, an 8-hour window, and a 100,000,000-token budget** (was 10 turns / 15 minutes / 200,000 tokens). The old turn ceiling stopped healthy long runs for no reason, and a run that has genuinely stopped working is caught within two turns by the no-tool-call and no-progress pauses regardless — so with these defaults those two pauses, with the 8-hour clock behind them, are the brakes that actually bind. `maxDurationMs` is `28800000` and `maxTokens` is `100000000`; every other option in the defaults block is unchanged.
...
- **`--max-turns` accepts every spelling of "no ceiling":** `0`, `unlimited`, `none`, `inf`, `infinite`, `infinity`, and `∞` (case-insensitive, and in the `--max-turns=…` form). A positive integer behaves exactly as before, and a garbage value still reports `Invalid positive integer for --max-turns: …` and leaves the configured default in place. `set_goal`/`goal_set` accept `maxTurns: 0` too; a negative or fractional count is still rejected. `/goal edit` deliberately does not parse flags — it takes the rest of the line verbatim as the new objective and preserves the existing budgets.
- **Rendering: `∞` for an unlimited turn budget, hours for a long clock.** The session title, `/goal status`, and the TUI sidebar panel all render the turn budget as `<used>/∞`, and durations of 60 minutes or more render in hours with a single decimal and no trailing `.0` — `45m`, `1h`, `1.5h`, `8h` (481 minutes is 8.016 h, which rounds to 8.0 and prints `8h`). Elapsed and limit are formatted independently, so a fresh 8-hour goal reads `0m/8h` and the same goal 90 minutes in reads `1.5h/8h`. The sidebar panel line goes from `1/1000 turns · 1/480m · 147k/100m tokens` to `1/∞ turns · 1m/8h · 147k/100m tokens`, and the session title from `▶ ship it · 3/10 · 2m · 45k/200k` to `▶ ship it · 3/∞ · 2m/8h · 45k/100m`. The title now carries the duration *limit* next to the elapsed clock, which it previously omitted.
...
- **`metadata.goal` payload change (schema still `v: 1`): `turns.max` is now `number | null`.** An unlimited budget is written as `{ "used": 3, "max": null, "unlimited": true }`; a bounded one is unchanged (`{ "used": 3, "max": 10 }`, no `unlimited` key). `minutes` and `tokens` stay plain numbers, so the payload remains machine-readable and the hour formatting lives only in the rendered strings. The sidebar panel treats a null ceiling as unlimited rather than dropping the whole stat, which is what it does for a genuinely absent budget. `GoalSidebarStatus` in `index.d.ts` and the `GoalPanelModel.stats` docs in `tui.d.ts` are updated to match.
