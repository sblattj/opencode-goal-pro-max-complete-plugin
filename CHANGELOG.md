...
- **`metadata.goal` payload is now `v: 2`: `turns.max` is `number | null`, and `durationMs` joins `minutes`.** An unlimited budget is written as `{ "used": 3, "max": null, "unlimited": true }`; a bounded one is unchanged (`{ "used": 3, "max": 10 }`, no `unlimited` key). New `durationMs: { used, max }` carries the exact millisecond duration the session title renders from, so the panel matches the title at every granularity — `minutes` alone could only say `0m/0m` for a sub-minute budget, and the panel dropped the duration stat entirely when the limit rounded to zero. Every v1 field is still written, and `minutes` now truncates like the rendered strings. The panel renders `durationMs` when present and falls back to `minutes`, and it treats **any** spelling of "no turn ceiling" as unlimited — `max: null`, `unlimited: true`, or `max: 0`, the value the rest of the package uses — rather than dropping the stat. Skew warning: the two halves are registered separately (`opencode.json` and `tui.json`), and a 0.10.x panel reading a v2 payload silently drops the turns stat, so upgrade both together. `GoalSidebarStatus` in `index.d.ts` and the `GoalPanelModel.stats` docs in `tui.d.ts` are updated to match.
- **Fix: the budget wrap-up handoff was unreachable under the new defaults.** `budgetWrapupNeeded` only watched the token budget, and `totalTokens` is the peak *context window* size — bounded by the model at 200k–2M — so a 100,000,000-token budget put the `<budget_wrapup>` prompt, the token brake, and the token warning permanently out of reach: a default goal would auto-continue against a saturated context for the full 8 hours with no handoff. The wrap-up now fires at `budgetWrapupRatio` of **either** the token budget or the duration window, whichever comes first, which is 6.4 h into the default 8-hour run. The turn budget is deliberately not a third dimension: it is unlimited by default, and a bounded one already gets a final handoff prompt at its ceiling. The token brake and token warning remain off by default *by design* — a long run should not end at its first context saturation, which the host handles by compacting — and `README.md` now says so instead of describing the budget as a live safety brake.
